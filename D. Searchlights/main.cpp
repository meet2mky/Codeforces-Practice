...
void solve()
{
    ...
    REP(i, 0, n)
    {
        REP(j, 0, m)
        {
            int dx = l[j].first - r[i].first + 1;
            int dy = l[j].second - r[i].second + 1;
            if (dx <= dxnow || dy <= dynow)
            {
                continue;
            }
            else
            {
                int mindyneed = dy - dynow;
                int mindxneed = dx - dxnow;
                if (mindxneed <= mindyneed)
                {
                    dxnow += mindxneed;
                }
                else
