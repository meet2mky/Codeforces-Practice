...
{
    int sq = pow((long double)num, 1.0 / 2);
    if (SQUARE(sq) == num)
        return sq;
    if (SQUARE(sq + 1) == num)
        return sq + 1;
...
int PERFECTCUBE(int num)
{
    int cb = pow((long double)num, 1.0 / 3);
    if (CUBE(cb) == num)
        return cb;
    if (CUBE(cb + 1) == num)
        return cb + 1;
...
void solve()
{
    int s1, s2;
    R(s1, s2);
    int a = PERFECTCUBE(s1 * s2);
    if (a == -1)
    {
        W("No");
        return;
    }
    int x = PERFECTSQUARE(a * s1 / s2), y = PERFECTSQUARE(a * s2 / s1);
    if (x == -1 || y == -1)
    {
        W("No");
        return;
